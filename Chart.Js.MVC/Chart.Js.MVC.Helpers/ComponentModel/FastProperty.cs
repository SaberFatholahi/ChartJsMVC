...
namespace Chart.Js.MVC.Helpers.ComponentModel
{
	...
	public class FastProperty
	{
		...
		/// </summary>
		public Action<object, object> ValueSetter => _valueSetter ?? (_valueSetter = MakeFastPropertySetter(Property));
			...
			ConcurrentDictionary<Type, IDictionary<string, FastProperty>> cache)
		{
			...
			if (!cache.TryGetValue(type, out fastProperties))
			{
				...
				fastProperties = candidates.Select(createPropertyHelper).ToDictionary(x => x.Name, StringComparer.OrdinalIgnoreCase);
